...
if USE_SRWLIB:
    try:
        try:
            import srwpy.srwlib as srwlib  # OASYS 2.0 - SRW 4
        except:
            import srwlib as srwlib  # SRW 3
    except:
        try:
...
def calc1d_pysru(bl, photonEnergyMin=3000.0, photonEnergyMax=55000.0, photonEnergyPoints=5,
    ...
    myUndulator = MagneticStructureUndulatorPlane(K=bl['Kv'], period_length=bl['PeriodID'],
                                                  length=bl['PeriodID'] * bl['NPeriods'])
    ...
    eArray, intensArray = simulation_test.calculate_spectrum_on_slit(abscissas_array=energies, use_eV=1,
                                                                     is_quadrant=is_quadrant, do_plot=0)
...
def calc1d_srw_on_axis(bl, photonEnergyMin=3000.0, photonEnergyMax=55000.0, photonEnergyPoints=500,
...
def calc1d_srw_parallel(bl, photonEnergyMin=3000.0, photonEnergyMax=55000.0, photonEnergyPoints=500,
                        zero_emittance=False,
                        srw_max_harmonic_number=None, fileName=None, fileAppend=False):
    r"""
        run SRW for calculating flux
    ...
    if USE_JOBLIB:
        ...
        for ptime in range(len(time_array)):
            print(
                f" Core {ptime + 1}: {time_array[ptime]:.2f} s for {energy_chunks[ptime]} pts (E0 = {energy_array[ptime]:.1f} eV).")
    else:
...
def calc1d_srw_step_by_step(bl, photonEnergyMin=3000.0, photonEnergyMax=55000.0, photonEnergyPoints=500,
                            zero_emittance=False,
                            srw_max_harmonic_number=None, fileName=None, fileAppend=False):
    r"""
        run SRW for calculating flux
...
def calc1d_urgent(bl, photonEnergyMin=1000.0, photonEnergyMax=100000.0, photonEnergyPoints=500, zero_emittance=False,
...
def calc1d_us(bl, photonEnergyMin=1000.0, photonEnergyMax=100000.0, photonEnergyPoints=500, zero_emittance=False,
              fileName=None, fileAppend=False):
    r"""
    ...
    for i in txt:
        ...
        if tmp[0].isdigit():
            ...
            if fileName is not None:
                f.write(
                    ("%g " * 8 + "\n") % (tmp[0], 1e10 * m2ev / tmp[0], tmp[1], tmp[1] * 1e3 * codata.e, tmp[2], tmp[3],
                                          tmp[4], tmp[5]))
            nArray += 1
        else:
            if fileName is not None: f.write("#UD " + tmp)
...
def calc2d_srw(bl, zero_emittance=False, hSlitPoints=101, vSlitPoints=51,
    ...
    stkP.allocate(1, hSlitPoints,
                  vSlitPoints)  # numbers of points vs horizontal and vertical positions (photon energy is not taken into account)
...
def calc3d_srw_step_by_step(bl, photonEnergyMin=3000.0, photonEnergyMax=55000.0, photonEnergyPoints=500,
    ...
    for ie in range(eArray.size):
        ...
        print("Calculating photon energy: %f (point %d of %d) time:%g" % (eArray[ie], ie + 1, eArray.size + 1,
                                                                          time.time() - t00))
        t00 = time.time()
    ...
    for ih in range(len(harmonics)):
        for i in range(Kvalues.size):
            print("%10d   %17.3f%17.3f%17.3f   %17.3g %17.3f" %
                  (int(harmonics[ih]), Kvalues[i], evalues[i, ih], evalues_at_flux_peak[i, ih],
                   flux_values[i, ih] * codata.e * 1e3, Pvalues[i]))
...
def _srw_drift_electron_beam(eBeam, und):
    if isinstance(und, float):
        length = und
                    ...
                    srwlib.SRWLMagFldU):  # Always defined in (0., 0., 0.) move the electron beam before the magnetic field.
...
def compare_flux(beamline, emin=3000.0, emax=50000.0, npoints=200,
    ...
    if USE_SRWLIB:
        e_s, f_s = calc1d_srw(beamline, photonEnergyMin=emin, photonEnergyMax=emax,
                              photonEnergyPoints=npoints, zero_emittance=zero_emittance, fileName=fileName,
                              fileAppend=True,
...
def compare_flux_from_3d(beamline, emin=3000.0, emax=50000.0, npoints=10,
    ...
    if USE_PYSRU:
        r_pysru = calc_from_3d("pySRU", beamline, photonEnergyMin=emin, photonEnergyMax=emax,
                               photonEnergyPoints=npoints,
                               npoints_grid=npoints_grid, zero_emittance=zero_emittance, fileName=fileName,
                               fileAppend=True)
    if USE_SRWLIB:
...
def compare_power_density(beamline, npoints_grid=40, zero_emittance=False, fileName=None, post_convolution=False):
    if post_convolution:
        ...
        from scipy.ndimage.filters import gaussian_filter1d as gaussian_filter1d
        SigmaH = numpy.sqrt(
            beamline['ElectronBeamSizeH'] ** 2 + (beamline['distance'] * beamline['ElectronBeamDivergenceH']) ** 2)
        SigmaV = numpy.sqrt(
            beamline['ElectronBeamSizeV'] ** 2 + (beamline['distance'] * beamline['ElectronBeamDivergenceV']) ** 2)
    ...
    if USE_US:
        h, v, p = calc2d_us(beamline, hSlitPoints=npoints_grid, vSlitPoints=npoints_grid, zero_emittance=zero_emittance,
                            fileName=fileName, fileAppend=True)
        if post_convolution:
    ...
    if USE_URGENT:
        h, v, p = calc2d_urgent(beamline, hSlitPoints=npoints_grid, vSlitPoints=npoints_grid,
    ...
    if USE_SRWLIB:
        h, v, p = calc2d_srw(beamline, hSlitPoints=npoints_grid, vSlitPoints=npoints_grid,
                             zero_emittance=zero_emittance, fileName=fileName, fileAppend=True)
        if post_convolution:
            p1 = gaussian_filter1d(p, SigmaH / (h[1] - h[0]), axis=0)
            p = gaussian_filter1d(p1, SigmaV / (v[1] - v[0]), axis=1)
    ...
    if USE_PYSRU:
        h, v, p = calc2d_pysru(beamline, hSlitPoints=npoints_grid, vSlitPoints=npoints_grid,
                               zero_emittance=zero_emittance, fileName=fileName, fileAppend=True)
        if post_convolution:
            p1 = gaussian_filter1d(p, SigmaH / (h[1] - h[0]), axis=0)
            p = gaussian_filter1d(p1, SigmaV / (v[1] - v[0]), axis=1)
    ...
    for key in ["calc2d_us", "calc2d_urgent", "calc2d_pysru", "calc2d_srw"]:
        if key in beamline_dict.keys():
            ...
            if contour: plot_contour(p, h, v, title="%s %s" % (beamline_dict['name'], key),
                                     ...
                                     contour_levels=contour_levels, cmap=None, cbar=1,
                                     cbar_title="Power density [$W/mm^2$]", show=0)
            if surface: plot_surface(p, h, v, title="%s %s" % (beamline_dict['name'], key), xtitle="H [mm]",
                                     ytitle="V [mm]", show=0)
    ...
    for key in ["calc3d_us", "calc3d_urgent", "calc3d_pysru", "calc3d_srw"]:
        if key in beamline_dict.keys():
            ...
            if stack:
                ...
                plot_contour(f[int(e.size / 2), :, :], h, v,
                             title="%s %s; E=%g eV" % (beamline_dict['name'], key, e[int(e.size / 2)]),
                             xtitle="H [mm]", ytitle="V [mm]", plot_points=0, contour_levels=contour_levels,
                             cmap=None, cbar=1, cbar_title="Flux ", show=False)

                plot_surface(f[int(e.size / 2), :, :], h, v,
                             title="%s %s; E=%g eV" % (beamline_dict['name'], key, e[int(e.size / 2)]),
...
def calculate_power(bl):
    for key in ["calc1d_us", "calc1d_urgent", "calc1d_pysru", "calc1d_srw"]:
        if key in bl.keys():
            ...
            f = bl[key]["flux"]
            print(
                ">>>>    Power from integral of spectrum (%s): %f W" % (key, f.sum() * 1e3 * codata.e * (e[1] - e[0])))
    ...
    for key in ["calc2d_us", "calc2d_urgent", "calc2d_pysru", "calc2d_srw"]:
        if key in bl.keys():
            ...
            v = bl[key]["v"]
            p = bl[key]["p"]
            print(">>>>    Power from power density calculations (%s): %f W" % (key, p.sum() * (h[1] - h[0]) * (
                        v[1] - v[0])))
...
if __name__ == '__main__':
    ...
    if False:  # check power density from harmonics
        ...
        if True:
            ...
            plot_image(POWER_DENSITY, X, Y, xtitle="H [mm]", ytitle="V [mm]",
                       title="Power density W/mm2 **FORTRAN FROM HARMONICS**", show=0)
            ...
            plot_image(POWER_DENSITY2, X2, Y2, xtitle="H [mm]", ytitle="V [mm]",
                       title="Power density W/mm2 **PYTHON FROM HARMONICS**", show=0)
